...
public abstract class Database {
        ...
        try {
            ...
            while (rs.next()) {
                if (rs.getString("playerName").equalsIgnoreCase(string.toLowerCase())) { // Tell database to search for the player you sent into the method. e.g getTokens(sam) It will look for sam.
                    return rs.getInt("totalTime"); // Return the players ammount of kills. If you wanted to get total (just a random number for an example for you guys) You would change this to total!
                }
            }
        } catch (SQLException ex) {
    ...
    public void setTokens(Player player, Integer tokens, Integer total) {
        ...
        try {
            conn = getSQLConnection();
            ps = conn.prepareStatement("REPLACE INTO " + table + " (playerUUID,playerName,totalTime) VALUES(?,?,?)"); // IMPORTANT. In SQLite class, We made 3 colums. player, Kills, Total.
